...
function Dashboard({ state, onNavigate }: { state: AppState; onNavigate: (view: View) => void }) {
  ...
  const fundingCategoryGroups = [...state.accounts.reduce((groups, account) => {
    const category = account.category ?? account.kind;
  ...
  return <>
    ...
    <section className="overview-summary-grid" aria-label="当前资金与资产摘要">
      <OverviewSnapshot title="当前资金" value={latestActual ? money(latestActual.totalMinor) : "—"} detail={latestActual ? `资金预测 · ACT · ${latestActual.month}（最新实际期间）` : "导入资金预测中的 ACT 数据后显示"} rows={[{ label: "灵活资金", value: latestActual ? money(latestActual.flexibleMinor) : "—" }, { label: "非灵活资金", value: latestActual ? money(latestActual.nonFlexibleMinor) : "—" }]} sections={fundingCategoryGroups.length ? [{ title: "资金分类（账户余额）", rows: fundingCategoryGroups.map(([label, amount]) => ({ label, value: money(amount), tone: amount < 0 ? "negative" as const : undefined })) }] : undefined} />
...
function OverviewSnapshot({ title, value, detail, rows, sections }: { title: string; value: string; detail: string; rows: { label: string; value: string; tone?: "warn" | "negative" }[]; sections?: { title: string; rows: { label: string; value: string; tone?: "warn" | "negative" }[] }[] }) {
  return <article className="card overview-snapshot"><div className="overview-snapshot-main"><span>{title}</span><strong className="money">{value}</strong><p>{detail}</p></div><div className="overview-snapshot-rows">{rows.map((row) => <div className="overview-snapshot-row" key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}</div>{sections?.map((section) => <div className="overview-snapshot-section" key={section.title}><span className="overview-snapshot-section-title">{section.title}</span>{section.rows.map((row) => <div className="overview-snapshot-row" key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}</div>)}</article>;
}
...
function CashflowRows({ cashflows, showFundingBucket = false }: { cashflows: ExpectedCashflow[]; showFundingBucket?: boolean }) { return <div className="forecast-list">{cashflows.map((flow) => <div className="forecast-row" key={flow.id}><span>{shortDate(flow.dueDate)}</span><span><strong>{flow.title}</strong>{showFundingBucket ? <span className={`cashflow-bucket ${flow.fundingBucket === "非灵活" ? "non-flexible" : "flexible"}`}>{flow.fundingBucket ?? "灵活"}资金</span> : <span className="footnote">{flow.category} · {flow.scenario}</span>}</span><span className={`money ${flow.direction === "流出" ? "negative" : ""}`}>{flow.direction === "流出" ? "-" : "+"}{money(flow.amountMinor, flow.currency)}</span></div>)}{cashflows.length === 0 && <p className="empty">没有待发生的预测现金流。</p>}</div>; }
