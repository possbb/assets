...
function Dashboard({ state, metrics, onNavigate }: { state: AppState; metrics: ReturnType<typeof calculateMetrics>; onNavigate: (view: View) => void }) {
  ...
  return <>
    <section className="metric-grid" aria-label="核心指标"><Metric label="家庭净资产" value={money(metrics.netWorth)} detail={`金融资产 ${money(metrics.financial)} · 已扣录入负债`} tone="good" /><Metric label="可动用资金" value={money(metrics.liquid)} detail={`占金融资产 ${((metrics.liquid / total) * 100).toFixed(1)}%`} tone={metrics.liquid / total < .2 ? "warn" : "good"} /><Metric label="未来90天预计现金流" value={`流出 ${money(metrics.next90Out)}`} detail={`流入 ${money(metrics.next90In)} · 待发生预测`} tone="warn" /><Metric label="需要处理事项" value={`${metrics.upcoming} 项`} detail="到期证照与30日内现金流" tone={metrics.upcoming ? "warn" : "good"} /></section>
