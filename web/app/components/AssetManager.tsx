...
type SnapshotRow = { label: string; value: string; tone?: "warn" | "negative"; details?: { label: string; value: string; tone?: "warn" | "negative" }[] };
function OverviewSnapshot({ title, value, detail, rows }: { title: string; value: string; detail: string; rows: SnapshotRow[] }) {
  const [expandedRow, setExpandedRow] = useState<string | null>(null);
  return <article className="card overview-snapshot"><div className="overview-snapshot-main"><span>{title}</span><strong className="money">{value}</strong><p>{detail}</p></div><div className="overview-snapshot-rows">{rows.map((row) => { const expanded = expandedRow === row.label; return <div className="overview-snapshot-row fixed-asset-group" key={row.label}><div className="fixed-asset-group-heading"><span>{row.label}</span>{row.details?.length ? <button type="button" className="expand-assets" aria-expanded={expanded} onClick={() => setExpandedRow(expanded ? null : row.label)}>{expanded ? "收起" : "展开"}</button> : null}</div><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong>{expanded && <div className="fixed-asset-details">{row.details?.map((detail) => <div key={detail.label}><span>{detail.label}</span><strong className={`money ${detail.tone ?? ""}`}>{detail.value}</strong></div>)}</div>}</div>; })}</div></article>;
}
...
function CashSafetyChart({ points }: { points: CashSafetyPoint[] }) {
  ...
  const mobileLabelEvery = Math.max(1, Math.ceil(visiblePoints.length / 8));
  ...
  const flexibleLabelIndexes = new Set(visiblePoints.flatMap((point, index) => {
    const monthNumber = Number(point.month.split("-")[1]);
    return [3, 6, 9, 12].includes(monthNumber) || index === flexibleExtremes.highest || index === flexibleExtremes.lowest ? [index] : [];
  }));
