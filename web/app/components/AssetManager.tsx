...
function CashSafetyChart({ points }: { points: CashSafetyPoint[] }) {
  ...
  const centerX = (index: number) => padding.left + (index + .5) / visiblePoints.length * plotWidth;
  const barWidth = Math.min(52, Math.max(10, plotWidth / visiblePoints.length * .72));
  ...
  return <section className="cash-safety-chart"><div className="cash-safety-legend"><span><i className="funding-stack-swatch non-flexible" />非灵活资金</span><span><i className="funding-stack-swatch flexible" />灵活资金</span><label className="funding-range">显示范围<select aria-label="资金趋势显示范围" value={selectedRange} onChange={(event) => { setActiveIndex(null); setRangeMonths(Number(event.target.value)); }}>{rangeOptions.map((months) => <option value={months} key={months}>近 {months} 个月</option>)}</select></label><span className="cash-safety-hint">悬停查看当月明细</span></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="按月连续堆积资金预测图，底层为非灵活资金，上层为灵活资金。" onMouseLeave={() => setActiveIndex(null)}><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={padding.top} y2={padding.top} /><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={padding.top + plotHeight / 2} y2={padding.top + plotHeight / 2} /><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={baseline} y2={baseline} /><text className="funding-trend-axis" x={padding.left - 10} y={padding.top + 4} textAnchor="end">{compact(maximum)}</text><text className="funding-trend-axis" x={padding.left - 10} y={baseline + 4} textAnchor="end">¥0</text>{visiblePoints.map((point, index) => { const flexibleHeight = barHeight(point.flexibleMinor); const nonFlexibleHeight = barHeight(point.nonFlexibleMinor); const activeBar = activeIndex === index; return <g key={point.month} className={activeBar ? "funding-stack active" : "funding-stack"} tabIndex={0} aria-label={`${point.month}，资金预测合计 ${money(point.totalMinor)}，灵活资金 ${money(point.flexibleMinor)}，非灵活资金 ${money(point.nonFlexibleMinor)}`} onMouseEnter={() => setActiveIndex(index)} onFocus={() => setActiveIndex(index)} onBlur={() => setActiveIndex(null)}><rect className="funding-stack-hit" x={centerX(index) - Math.max(barWidth, 28) / 2} y={padding.top} width={Math.max(barWidth, 28)} height={plotHeight} /><rect className="funding-stack-non-flexible" x={centerX(index) - barWidth / 2} y={baseline - nonFlexibleHeight} width={barWidth} height={nonFlexibleHeight} rx="3" /><rect className="funding-stack-flexible" x={centerX(index) - barWidth / 2} y={baseline - nonFlexibleHeight - flexibleHeight} width={barWidth} height={flexibleHeight} rx="3" /><text className="funding-trend-axis" x={centerX(index)} y={height - 16} textAnchor="middle">{index % Math.max(1, Math.ceil(visiblePoints.length / 12)) === 0 || index === visiblePoints.length - 1 ? point.month.slice(5) : ""}</text></g>; })}{active && <g className="funding-stack-tooltip" transform={`translate(${tooltipX},${padding.top + 10})`} pointerEvents="none"><rect width="178" height="84" rx="8" /><text x="12" y="19" className="funding-stack-tooltip-title">{active.month}</text><text x="12" y="39">资金预测合计 {money(active.totalMinor)}</text><text x="12" y="57">灵活资金 {money(active.flexibleMinor)}</text><text x="12" y="75">非灵活资金 {money(active.nonFlexibleMinor)}</text></g>}</svg></section>;
}
