...
function OverviewSnapshot({ title, value, detail, rows }: { title: string; value: string; detail: string; rows: { label: string; value: string; tone?: "warn" | "negative" }[] }) {
  return <article className="card overview-snapshot"><div className="overview-snapshot-main"><span>{title}</span><strong className="money">{value}</strong><p>{detail}</p></div><div className="overview-snapshot-rows">{rows.map((row) => <div key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}</div></article>;
}
...
function cashSafetyPoints(state: AppState): CashSafetyPoint[] {
  if (state.fundingForecast?.length) return state.fundingForecast.slice(0, 13).map((point) => ({ month: point.month, flexibleMinor: point.flexibleMinor, nonFlexibleMinor: point.nonFlexibleMinor, totalMinor: point.totalMinor }));
  const liquid = state.accounts.filter((account) => account.liquidity === "高").reduce((sum, account) => sum + account.balanceMinor, 0);
...
function CashSafetyChart({ points }: { points: CashSafetyPoint[] }) {
  ...
  const width = 920; const height = 316; const padding = { top: 22, right: 24, bottom: 46, left: 78 };
  ...
  const maximum = Math.max(1, ...points.map((point) => Math.max(point.totalMinor, point.flexibleMinor + point.nonFlexibleMinor)));
  const centerX = (index: number) => padding.left + (index + .5) / points.length * plotWidth;
  const barWidth = Math.min(52, Math.max(16, plotWidth / points.length * .64));
  const barHeight = (value: number) => Math.max(0, value / maximum * plotHeight);
  ...
  const tooltipX = activeIndex === null ? 0 : Math.max(padding.left, Math.min(width - padding.right - 178, centerX(activeIndex) - 82));
  return <section className="cash-safety-chart"><div className="cash-safety-legend"><span><i className="funding-stack-swatch flexible" />灵活资金</span><span><i className="funding-stack-swatch non-flexible" />非灵活资金</span><span className="cash-safety-hint">悬停查看当月明细</span></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="按月堆积资金预测图，显示灵活资金与非灵活资金。" onMouseLeave={() => setActiveIndex(null)}><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={padding.top} y2={padding.top} /><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={padding.top + plotHeight / 2} y2={padding.top + plotHeight / 2} /><line className="funding-trend-grid" x1={padding.left} x2={width - padding.right} y1={baseline} y2={baseline} /><text className="funding-trend-axis" x={padding.left - 10} y={padding.top + 4} textAnchor="end">{compact(maximum)}</text><text className="funding-trend-axis" x={padding.left - 10} y={baseline + 4} textAnchor="end">¥0</text>{points.map((point, index) => { const flexibleHeight = barHeight(point.flexibleMinor); const nonFlexibleHeight = barHeight(point.nonFlexibleMinor); const activeBar = activeIndex === index; return <g key={point.month} className={activeBar ? "funding-stack active" : "funding-stack"} tabIndex={0} aria-label={`${point.month}，资金预测合计 ${money(point.totalMinor)}，灵活资金 ${money(point.flexibleMinor)}，非灵活资金 ${money(point.nonFlexibleMinor)}`} onMouseEnter={() => setActiveIndex(index)} onFocus={() => setActiveIndex(index)} onBlur={() => setActiveIndex(null)}><rect className="funding-stack-hit" x={centerX(index) - Math.max(barWidth, 34) / 2} y={padding.top} width={Math.max(barWidth, 34)} height={plotHeight} /><rect className="funding-stack-flexible" x={centerX(index) - barWidth / 2} y={baseline - flexibleHeight} width={barWidth} height={flexibleHeight} rx="3" /><rect className="funding-stack-non-flexible" x={centerX(index) - barWidth / 2} y={baseline - flexibleHeight - nonFlexibleHeight} width={barWidth} height={nonFlexibleHeight} rx="3" /><text className="funding-trend-axis" x={centerX(index)} y={height - 16} textAnchor="middle">{index % Math.max(1, Math.ceil(points.length / 12)) === 0 || index === points.length - 1 ? point.month.slice(5) : ""}</text></g>; })}{active && <g className="funding-stack-tooltip" transform={`translate(${tooltipX},${padding.top + 10})`} pointerEvents="none"><rect width="178" height="84" rx="8" /><text x="12" y="19" className="funding-stack-tooltip-title">{active.month}</text><text x="12" y="39">资金预测合计 {money(active.totalMinor)}</text><text x="12" y="57">灵活资金 {money(active.flexibleMinor)}</text><text x="12" y="75">非灵活资金 {money(active.nonFlexibleMinor)}</text></g>}</svg></section>;
}
