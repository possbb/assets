...
function Dashboard({ state, onNavigate }: { state: AppState; onNavigate: (view: View) => void }) {
  ...
  const fundingCategoryGroups = (["灵活", "非灵活"] as const).map((fundingNature) => ({
    ...
    rows: [...state.accounts.filter((account) => account.category && (account.fundingNature ?? (account.liquidity === "低" ? "非灵活" : "灵活")) === fundingNature).reduce((groups, account) => {
      ...
      return groups;
    }, new Map<string, Account[]>()).entries()].map(([category, accounts]) => ({ category, accounts, amount: accounts.reduce((sum, account) => sum + account.balanceMinor, 0) })).sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount)),
  }));
  return <>
...
type SnapshotRow = { label: string; value: string; tone?: "warn" | "negative" };
function OverviewSnapshot({ title, value, detail, rows, sections }: { title: string; value: string; detail: string; rows: SnapshotRow[]; sections?: { title: string; className?: string; rows: SnapshotRow[] }[] }) {
  const [expandedSection, setExpandedSection] = useState<number | null>(null);
  const rowTemplate = sections?.map((section, index) => `${expandedSection === index ? Math.max(1, section.rows.length) : 1}fr`).join(" ");
  return <article className="card overview-snapshot"><div className="overview-snapshot-main"><span>{title}</span><strong className="money">{value}</strong><p>{detail}</p></div><div className="overview-snapshot-rows" style={rowTemplate ? { gridTemplateRows: rowTemplate } : undefined}>{rows.map((row, index) => { const canExpand = Boolean(sections?.[index]?.rows.length); const expanded = expandedSection === index; return <div className="overview-snapshot-row" key={row.label}><div className="snapshot-row-heading"><span>{row.label}</span>{canExpand && <button type="button" className="expand-assets" aria-expanded={expanded} onClick={() => setExpandedSection(expanded ? null : index)}>{expanded ? "收起" : "展开"}</button>}</div><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>; })}</div>{sections && <div className="overview-snapshot-sections" style={{ gridTemplateRows: rowTemplate }}>{sections.map((section, index) => <div className={`overview-snapshot-section ${section.className ?? ""}`} key={`${section.title}-${index}`}>{section.title && <span className="overview-snapshot-section-title">{section.title}</span>}{expandedSection === index && section.rows.map((row) => <div className="overview-snapshot-row" key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}</div>)}</div>}</article>;
}

function FixedAssetSnapshot({ assets, groups, total }: { assets: Asset[]; groups: [string, number][]; total: number }) {
  const [expandedCategory, setExpandedCategory] = useState<string | null>(null);
  return <article className="card overview-snapshot fixed-asset-snapshot"><div className="overview-snapshot-main"><span>固定资产和股票期权</span><strong className="money">{money(total)}</strong><p>仅统计“固定资产和股票期权”工作表导入的资产；按资产分类汇总。</p></div><div className="overview-snapshot-rows">{groups.map(([category, amount]) => { const expanded = expandedCategory === category; const rows = assets.filter((asset) => (asset.category ?? asset.type) === category); return <div className="overview-snapshot-row fixed-asset-group" key={category}><div className="fixed-asset-group-heading"><span>{category}</span><button type="button" className="expand-assets" aria-expanded={expanded} onClick={() => setExpandedCategory(expanded ? null : category)}>{expanded ? "收起" : "展开"}</button></div><strong className="money">{money(amount)}</strong>{expanded && <div className="fixed-asset-details">{rows.map((asset) => <div key={asset.id}><span>{asset.name}</span><strong className="money">{money((asset.grossValueMinor - asset.liabilityMinor) * asset.ownershipPct / 100, asset.currency)}</strong></div>)}</div>}</div>; })}</div></article>;
}
