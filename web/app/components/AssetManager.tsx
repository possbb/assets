...
function OverviewSnapshot({ title, value, detail, rows, sections }: { title: string; value: string; detail: string; rows: { label: string; value: string; tone?: "warn" | "negative" }[]; sections?: { title: string; rows: { label: string; value: string; tone?: "warn" | "negative" }[] }[] }) {
  return <article className="card overview-snapshot"><div className="overview-snapshot-main"><span>{title}</span><strong className="money">{value}</strong><p>{detail}</p></div><div className="overview-snapshot-rows">{rows.map((row) => <div className="overview-snapshot-row" key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}{sections?.map((section) => <div className="overview-snapshot-section" key={section.title}><span className="overview-snapshot-section-title">{section.title}</span>{section.rows.map((row) => <div className="overview-snapshot-row" key={row.label}><span>{row.label}</span><strong className={`money ${row.tone ?? ""}`}>{row.value}</strong></div>)}</div>)}</div></article>;
}
